...
sub encodePlayerPicks {
	...
	if ($man_or_chimp eq 'both') {
		...
		$players_sql = "select * from player_info where man_or_chimp = '$man_or_chimp' order by player_id";
	}
...
sub getCurrentPlayerScores {
	...
	if ($man_or_chimp eq 'both' ) {
		$players_sql = "select player_info.*, scores.score from player_info, scores where player_info.player_id = scores.player_id and step = '$step'";
	}
	else {
		$players_sql = "select player_info.*, scores.score from player_info, scores where player_info.player_id = scores.player_id and player_info.man_or_chimp = '$man_or_chimp' and step = '$step'";
	}
