...
my $currentScoreRef = getCurrentPlayerScores($man_or_chimp);
...
my $playerPicks = encodePlayerPicks($man_or_chimp);
...
sub encodePlayerPicks {
	...
	if ($man_or_chimp eq 'both') {
		#$players_sql = "select * from player_info, filter, filter_link where player_info.player_id = filter_link.player_id and filter.filter_id = $filter order by player_info.player_id";
        $players_sql = "SELECT * from player_info JOIN filter_link fl ON player_info.player_id = fl.player_id JOIN filter f ON fl.filter_id = f.filter_id where f.filter_id = $filter";
	}
	else {
#		$players_sql = "select * from player_info, filter, filter_link where player_info.player_id = filter_link.player_id and filter.filter_id = $filter where man_or_chimp = '$man_or_chimp' order by player_info.player_id";
        $players_sql = "SELECT * from player_info JOIN filter_link fl ON player_info.player_id = fl.player_id JOIN filter f ON fl.filter_id = f.filter_id where f.filter_id = $filter and player_info.man_or_chimp = '$man_or_chimp'";
	}
...
#		my $query = "SELECT score from player_info, scores, filter, filter_link where player_info.player_id = filter_link.player_id and filter.filter_id = $filter and player_info.player_id = $player and scores.step = '$step'";
        my $query = "SELECT score FROM scores JOIN player_info pi ON scores.player_id = pi.player_id JOIN filter_link fl ON pi.player_id = fl.player_id JOIN filter f ON fl.filter_id = f.filter_id where f.filter_id = $filter and scores.step = $step and scores.player_id = '$player'";
        print $query . "\n";
		my $score = single_row_query($query);
...
sub getCurrentPlayerScores {
	...
	if ($man_or_chimp eq 'both' ) {
#		$players_sql = "select player_info.*, scores.score from player_info, scores, filter_link, filter where filter.filter_id = $filter and filter_link.player_id = player_info.player_id and player_info.player_id = scores.player_id and step = '$step'";
        $players_sql = "select * from player_info JOIN filter_link f1 ON player_info.player_id JOIN filter f on f1.filter_id = f.filter_id where f.filter_id = $filter";
	}
	else {
		#$players_sql = "select player_info.*, scores.score from player_info, scores, filter_link, filter where filter.filter_id = $filter and filter_link.player_id = player_info.player_id and player_info.player_id = scores.player_id and player_info.man_or_chimp = '$man_or_chimp' and step = '$step'";
        $players_sql = "select * from player_info JOIN filter_link f1 ON player_info.player_id JOIN filter f on f1.filter_id = f.filter_id where f.filter_id = $filter";
	}
