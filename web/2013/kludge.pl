...
#	print "$score_sql\n";
	...
	if ($manorchimp eq 'man') {
		open(FOREMAIL,">$data_dir/top10.txt") or warn "can't open top10.txt: $!";
		$place = 1;
		...
		close(FOREMAIL);
		chmod 0777, "$data_dir/top10.txt" or warn "couldn't chmod top10.txt: $!";
		open(WRITE,">$data_dir/top10_abbr.txt") or warn "can't write top10_abbr.txt: $!";
		for my $data (@sorted_score_data) {
			last if $data->{'rank'} > 10;
		...
		close(WRITE);
		chmod 0777, "$data_dir/top10_abbr.txt" or warn "can't do that: $!";

		# make histogram of scores
#	open (PROG, "/usr/bin/perl /data/cgi-bin/histogram.pl |") or die "couldn't do it: $!";
#	while (<PROG>) {
#        	print;
#	}
#	close(PROG);
