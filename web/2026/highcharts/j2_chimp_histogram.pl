...
######################################
 ...
 my $histogram = GD::Graph::bars->new(640,200);
  ...
  $histogram->set( 
         ...
         x_label_skip => 10,
         y_label => "Frequency",
         logo => "$webdir/jqmcbp/graphomatic/jq_graph_logo.png",
         logo_resize => 1,
         logo_position => 'UR',
         );
...
print STDOUT "$biggest\n";
  ...
  open(IMG1, ">$png_path/chimp_jfactor_histogram.png") or die $!;
     binmode IMG1;
     print IMG1 $gd->png;
...
my $query = "select j2_factor from player_info where man_or_chimp = \"chimp\" order by j2_factor";
print "$query\n";
