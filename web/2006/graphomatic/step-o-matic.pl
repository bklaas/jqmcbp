...
my $logo_path ="/etc/httpd/htdocs/johnnyquest/images/jq_graph_logo.gif";
my $font_path = "/usr/share/fonts/TTF";
my @data;
...
my @scores;
my ($name_href, $name_aref) = grab_names(\@ids);
my %names = %$name_href;
my @names = @$name_aref;
my $steps = get_steps();
my @steps = @$steps;
...
print STDOUT "$graphname_stub$statistic.png\n";
...
sub get_steps {
	...
	my $query = 'select step from scores where player_id = 1 order by step';
	my $ref = multi_row_query($query);
	my @return;
	for (@$ref) {
		push @return, $_->{'step'};
