...
for my $year (2005..2008) {
	...
	$stamps = get_timestamps_from_db($db);
...
my @graph_data = ( [ @$labels ] , [ @$aref1 ] , [ @$aref2 ], [ @$aref3 ], [ @$aref4 ] );

open(OUT, ">test.txt");
my $i = 0;
for my $elem (@$labels) {
	print OUT "$elem,$aref1->[$i],$aref2->[$i],$aref3->[$i],$aref4->[$i]\n";
	$i++;
}
exit;
...
$graph->set(
	...
	long_ticks => 1,
        transparent	=> 0,
	dclrs	=> [ qw(blue green red purple black) ],
	bgclr	=> "white",
