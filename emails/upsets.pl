...
for (sort keys %upsets) {
	my $query = "select picks.name, picks.winner from picks, player_info where picks.player_id = player_info.player_id and game = \"$_\" and winner = \"$upsets{$_}\" and (man_or_chimp = 'man' or man_or_chimp = 'celebrity') order by name";
#	print $query . "\n";
	print "$upsets{$_}:  ";
